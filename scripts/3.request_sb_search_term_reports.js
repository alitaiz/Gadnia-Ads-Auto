// scripts/3.request_sb_search_term_reports.js
import pkg from 'pg';
const { Pool } = pkg;
...
const REPORT_TYPE_ID = 'sbSearchTerm';

// --- API & DB Logic ---

const createReportRequest = async (accessToken, dateStr) => {
    console.log(`[API] ➡️  Requesting SB report from Amazon for date: ${dateStr}`);
    ...
    const reportRequestBody = {
        name: `SB Search Term Report for ${dateStr}`,
        startDate: dateStr,
        endDate: dateStr,
        configuration: {
    ...
    if (!response.ok) {
        throw new Error(`Failed to create SB report for ${dateStr}: ${JSON.stringify(data)}`);
    }
...
const saveRequestToDB = async (client, reportId, dateStr) => {
    ...
    console.log(`[DB] 💾 Saved SB request to queue. Report ID: ${reportId} for Date: ${dateStr}`);
};
...
const main = async () => {
    let client;
    try {
        console.log('🚀 Starting Phase 1: Bulk SB Report Requester...');
        
        const args = process.argv.slice(2);
        if (args.length !== 2) {
            console.error('❌ Usage: node scripts/3.request_sb_search_term_reports.js YYYY-MM-DD YYYY-MM-DD');
            process.exit(1);
        }
        ...
        console.log(`[Orchestrator] Requesting SB reports for dates from ${startArg} to ${endArg}.`);
        ...
        for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
            ...
            try {
                ...
                console.error(`💥 Failed to process SB report for date ${currentDateStr}: ${error.message}`);
            }
        }

        console.log('\n🎉 Phase 1 Complete for SB reports. All requests have been queued.');
        console.log('   Run the processing script to download completed reports.');

    } catch (error) {
        console.error('\n💥 A critical error occurred during the SB request process:', error);
        process.exit(1);
    } finally {
        if (client) client.release();
        await pool.end();
        console.log('👋 SB Requester script finished.');
    }
};
